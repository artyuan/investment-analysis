...
from app.utils import stock_list, MyStrategy
...
def get_rsi_strategy(ind, high_threshold):
  ...
  return ref_list
def get_stock_signal(ticker, best_stock_strategy):
    df = pd.DataFrame()
    ...
    return df_filter
def update_all_ticker_signals(df):
    df_signals = pd.DataFrame()
    for ticker in tqdm(stock_list):
        ticker_signal = get_stock_signal(ticker)
        df_signals = pd.concat([df_signals, ticker_signal], axis=0)
    df_signals['last_updated_date'] = datetime.today().date()
    df = pd.concat([df, df_signals])
    df.to_csv('../data/daily_update.csv', index=False)
    return df
...
if __name__ == '__main__':
    ...
    daily_update = pd.read_csv('../data/daily_update.csv')
    ...
    for ticker in tqdm(stock_list):
        ...
        df_signals = pd.concat([df_signals, ticker_signal], axis=0)
    df_signals['last_updated_date'] = datetime.today().date()
    daily_update = pd.concat([daily_update, df_signals])
    daily_update.to_csv('../data/daily_update.csv', index=False)
